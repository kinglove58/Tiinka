const servicesDataList = [
  {
    id: "Insomnia",
    name: "Insomnia",
    id_sub: "Find restful sleep and peace of mind",
    title1: "Tinka Health will help you tackle insomnia",
    title1Des:
      "Lack of sleep can turn your world upside down. Insomnia isn’t just frustrating; it impacts every part of your life, from work productivity to emotional stability. If you’re struggling with sleepless nights, Tinka Health is here to help you overcome insomnia and regain restful sleep.",
    btnText: "Get My Sleep Back",
    title2: "Is insomnia taking a toll on your life?",
    title2Des:
      "If you find yourself lying awake for hours, frustrated, anxious, or overwhelmed by endless thoughts, you’re not alone. Many people try to solve insomnia by “just relaxing” or ignoring it, but without addressing its root causes, the cycle often continues night after night and this can lead to repercussions such as:",
    title2List: [
      "Daytime fatigue and difficulty concentrating",
      "Strained relationships due to irritability or withdrawal",
      "Increased risk of mental health issues, such as anxiety or depression",
      "A weakened immune system, leading to frequent illness",
      "Dependence on sleep medications that may lose effectiveness over time",
    ],
    title3: "Goals of Insomnia Therapy",
    title3Des:
      "The goal of insomnia therapy with Tinka Health is to uncover the causes of your sleeplessness and provide effective tools for better rest. When you work with a Tinka Health therapist, you’ll learn how to:",
    title3List: [
      "Identify patterns and triggers for insomnia through journaling or sleep tracking.",
      "Address any underlying mental or physical health issues that might be contributing to sleep disruptions.",
      "Develop a bedtime routine and relaxation techniques tailored to your needs, helping you ease into restful sleep naturally.",
      "Manage anxious thoughts and promote a healthy mindset about sleep, reducing sleep-related stress.",
      "Improve lifestyle habits that contribute to better overall sleep quality, like diet and mindfulness practices",
    ],
  },
  {
    id: "Individual-Therapy",
    name: "Individual Therapy",
    id_sub: "Grow into your best, authentic self",
    title1: "Let’s smash your therapy goals together",
    title1Des:
      "At Tinka Health, we believe that successful therapy starts with a therapist who truly understands and supports you. We’re here to personally guide you on your journey to growth and healing, meeting you where you are and addressing your unique needs.",
    btnText: "Let's Do It",
    title2: "What you can achieve with individual therapy",
    title2Des:
      "Individual therapy is the foundation of mental health care and self-discovery. Together with Tinka Health, we’ll delve into your experiences, goals, challenges, and dreams to uncover solutions that bring real change. Here’s how Tinka Health supports your journey:",
    title2List: [
      "Develop strategies to transform unhelpful thought patterns and behaviors.",
      "Strengthen interpersonal skills and create meaningful relationships.",
      "Rediscover and embrace your authentic self, both inside and out.",
      "Access effective treatment for issues like PTSD, depression, anxiety, or ADHD.",
      "Set actionable short-term and long-term goals for your healing journey.",
      "Learn coping techniques for challenging life events like divorce, loss, or trauma.",
      "Establish healthy boundaries in relationships, work, and personal areas.",
    ],
    title3: "Tinka Health’s promise for your mental health journey",
    title3Des:
      "We understand therapy may feel daunting, especially if past experiences didn’t meet your expectations. Tinka Health is committed to making your experience different and effective. Our goal is to help you overcome obstacles, achieve your goals, and live a fulfilling life.",
    title3List: [
      "Tinka Health is dedicated to helping you break through challenges, using creative and personalized approaches that work best for you.",
      "Your sessions are entirely confidential, ensuring your trust and safety.",
      "At Tinka Health, we value authenticity. We don’t aim to change who you are; instead, we support the real you in thriving.",
      "Therapy can be challenging, that is why we focus mainly on teletherapy to make each session as comfortable as possible.",
    ],
  },
  {
    id: "Couples-Therapy",
    name: "Couples Therapy",
    id_sub: "Strengthen and nurture your relationship",
    title1: "Get support in navigating your relationship ",
    title1Des:
      "Whether you’re dating, engaged, or married, relationships can be challenging. If you and your partner are experiencing a rough patch, are considering separating, or simply want to become better communicators and partners in life, Tinka Health can help you.",
    btnText: "Let's Do It",
    title2: "What you can achieve with couples therapy",
    title2Des:
      "Couples therapy involves three people: you, your partner, and your therapist. Relationships can get messy! Tinka Health can help you identify what’s causing a breakdown and guide you in navigating the road ahead. Here is how we can support you and your partner on this journey.",
    title2List: [
      "Work through infidelity, dishonesty, and other trust-breaking behaviors.",
      "Open the door for an intimate, honest conversation with your partner.",
      "Discuss how outside parties are negatively impacting your relationship.",
      "Get support in navigating loss, traumatic situations, or difficult life changes.",
      "Set healthy boundaries with your partner, in-laws, and work. ",
      "Identify negative communication and behavioral patterns in the relationship. ",
      "Address relationship disappointment, hurt, resentment, and trauma.",
    ],
    title3: "Tinka Health’s promise for your mental health journey",
    title3Des:
      "Relationships are HARD, but you don’t have to face the challenges alone. Whether you’re working through a rough patch or need help addressing significant issues, Tinka Health is here to support you and your partner. Couples counseling can strengthen your bond, improve communication, and help you find lasting satisfaction together.",
    title3List: [
      "Tinka Health is dedicated to helping you and your partner navigate complex issues and strengthen your bond. We’re always ready to think creatively to help you grow together.",
      "Your trust is our priority. We ensure total privacy, and we will never share information or choose sides without your consent.",
      "We embrace what makes both you and your partner unique. Tinka Health helps you grow as a couple while honoring the individuality each of you brings to the relationship.",
      "Tinka Health offers a safe, comfortable virtual environment, allowing you and your partner to address challenges from the privacy of your own space, wherever you feel most at ease.",
    ],
  },
  {
    id: "Bipolar-Disorder",
    name: "Bipolar Disorder",
    id_sub: "Manage mood swings and live with stability",
    title1: "Get support for managing bipolar disorder with Tinka Health",
    title1Des:
      "If you are living with bipolar disorder, you might feel like a slave to your moods and emotions. At some points, you feel depressed, unmotivated, and withdrawn. However, at other times, you feel elated, impulsive, restless, agitated, and full of self-confidence. This emotional instability can greatly impact your life but Tinka Health is here to help you stabilize your moods and make healthier choices.",
    btnText: "Let's Do It",
    title2: "Does bipolar disorder make your moods unpredictable? ",
    title2Des:
      "Bipolar disorder is characterized by mood swings between hypomania, mania, and major depressive episodes, with types ranging from Bipolar I (severe mania sometimes requiring hospitalization) to Bipolar II (major depressive episodes with less severe hypomania) and cyclothymia (frequent, less intense mood shifts). Manic episodes may bring heightened energy, irritability, and impulsive behaviors like overspending or risk-taking, which, despite the temporary high, often lead to serious consequences. including:",
    title2List: [
      "Emotional crashes after manic episodes",
      "Emotional crashes after manic episodes",
      "Impulsive actions that might result in harm or regret",
      "Substance use as a coping mechanism",
      "Struggles with relationships",
      "Fluctuations in sleep, appetite, and energy",
      "Poor self-image or identity crises",
    ],
    title3: "Tinka Health’s approach to therapy for bipolar disorder",
    title3Des:
      "Bipolar disorder affects everyone differently, and Tinka Health understands that your experience is unique. Our goal is to help you regulate your mood and regain control of your mental health. Here’s how Tinka Health can support you:",
    title3List: [
      "Explore treatment options, including lifestyle changes, talk therapy, and potentially medication, to help you manage mood swings.",
      "Reduce impulsive or risky behaviors that could impact your relationships, health, finances, or work.",
      "Address depressive feelings, social withdrawal, and thoughts of self-harm.",
      "Decrease substance use during mood episodes and find healthier coping mechanisms.",
      "Understand how bipolar disorder affects your life and develop strategies to navigate periods of mania and depression effectively.",
    ],
  },
  {
    id: "Childrens-Therapy",
    name: "Childrens Therapy",
    id_sub: "Support and guidance for young minds",
    title1: "Play therapy and talk therapy for kids",
    title1Des:
      "Everyone needs help with their mental health sometimes – including kids. If your child is showing signs of a behavioral issue or is struggling with their emotional health, you don’t need to do this alone. Tinka Health is the perfect fit for you and your child.",
    btnText: "Let's Do It",
    title2: "What you can achieve with children’s therapy",
    title2Des:
      "It can be difficult to watch your child struggle with their mental health. The good news is that getting your child the mental health care they need now can have a positive impact on their future. Here’s how we can help your child thrive with children’s therapy.",
    title2List: [
      "Help your child develop confidence and grow into their unique, authentic self.",
      "Work alongside parents to identify opportunities for growth and connection.",
      "Utilize play therapy to help your child more comfortably express themselves.",
      "Support your child in developing healthy coping skills when they struggle. ",
      "Show your child how to identify and address extreme or difficult emotions. ",
      "Teach your child about their mental health and that feeling bad is okay.",
      "Help your child improve their communication and interpersonal skills.",
    ],
    title3:
      "Tinka Health’s promise for you and your child’s mental health care",
    title3Des:
      "If your child is struggling, you might blame yourself and wonder what you could have done differently to better support them. The reality is that many children have mental health or behavioral issues, and it’s hard for kids to understand and express their emotions. We’re not here to judge or overstep your role as a parent. Rather, we want to work with you to help your child reach their full potential. We’re dedicated to providing your child with exceptional mental health care and supporting you on this journey.",
    title3List: [
      "No child is too difficult or a lost cause. We explore any and every avenue to help your child heal. We’re not afraid to think outside of the box and use creative treatment methods that are customized to your child.",
      "Your child has a unique perspective on life, and they are still figuring out their identities, their dreams, their values, and their quirks. We’ll help your child explore who they are and learn to love and accept their authentic self. ",
      "Therapy can be intimidating for kids, but at Tinka Health, our virtual therapists work hard to create a comfortable online environment. We won’t push your child beyond their limits and focus on building a connection that feels safe and engaging. Our approach often includes play therapy, allowing your child to express themselves in a way that suits them best, all from the comfort of home.",
    ],
  },
  {
    id: "Anxiety-Disorders",
    name: "Anxiety Disorders",
    id_sub: "Overcome fears and regain control",
    title1: "we are here to help you tackle that anxiety",
    title1Des:
      "When you live with an anxiety disorder, your thoughts may feel hectic, overwhelming, and out of control. Fear and stress may overtake your mind and make it difficult to live a peaceful, fulfilling life. And while you would love to simply “get over it” and stop worrying so much, at Tinka Health, we know it’s never that simple. Let Tinka Health helps you regain control and conquer your fears. ",
    btnText: "Let's Do It",
    title2: "Is anxiety keeping you from living your best life?",
    title2Des:
      "How has anxiety prevented you from living your dreams? Has social anxiety held you back from trying new things or making friends? Do you waste hours worrying about what-ifs or find that a phobia limits your family's activities? To others, your anxiety might seem trivial, but for those living with an anxiety disorder, fears and feelings of dread can feel insurmountable, making it seem nearly impossible to confront your worries and live freely. Here are just a few of the ways anxiety can severely impact your life: ",
    title2List: [
      "Feeling worried about everything all the time ",
      "Experiencing physical pain and muscle tension ",
      "Having difficulties sleeping, eating, and concentrating",
      "Experiencing frequent panic attacks, often without warning",
      "Avoiding certain places, activities, people, animals, or situations ",
      "Feeling excessively flustered or self-conscious around others",
      "Having frequent nightmares, flashbacks, or intrusive memories",
      "Feeling irritable, restless, moody, or always on edge",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "Everyone feels anxious from time to time – and for a good reason. If not, how else would you know it’s not a good idea to pet wild bears or run out in front of a car? However, when anxiety consumes many of your thoughts and limits your potential, your mental health begins to suffer. And you might have great reason to be anxious. Maybe you experienced a traumatic event in the past or were exposed to something scary as a kid. We want to help you gain control of your anxiety so you can live life on your own terms – unruled by fear and unconsumed by worry. We are here to help you: ",
    title3List: [
      "List the people, places, and things that make you anxious, and begin to understand why you feel that way.",
      "Help you expose yourself to your fears and phobias in a slow, controlled manner that you’re comfortable with.  ",
      "Understand how anxiety is impacting your relationships, work life, mental health, physical health, and overall life satisfaction. ",
      "Identify your goals that anxiety is getting in the way of and make actionable steps toward achieving those goals. ",
      "Working through negative thought patterns and coping mechanisms and helping you gain control of your thoughts, emotions, and actions.",
    ],
  },
  {
    id: "Depression",
    name: "Depression",
    id_sub: "Rediscover hope and resilience",
    title1: "we are here to help you tackle that depression",
    title1Des:
      "Depression is one of the most common mental health disorders people struggle with, but depression doesn’t look the same for everyone. You need to join forces with Tinka Health to learn to understand and treat you like the unique human you are.",
    btnText: "Let's Do It",
    title2: "Feel overwhelmed, alone, or completely numb?",
    title2Des:
      "There are many reasons you might experience depression, including brain irregularities, trauma, negative thought patterns, and significant life disappointments. This condition can lead to persistent negative thoughts, feelings of overwhelm, and a sense that life isn't worth living. Depression can come in episodes or linger, you may notice a range of symptoms that hinder your ability to live a happy and productive life:",
    title2List: [
      "Lack of pleasure, joy, passion, or positive emotions  ",
      "Brain fog, difficulties concentrating, and feeling disconnected",
      "Overwhelming negative emotions, irritability, and anger",
      "Self-loathing, intense guilt or shame, and hopelessness",
      "Physical pain, including body aches, headaches, and nausea",
      "Feelings like you have no control or no way out",
      "Changes in your sleep, diet, self-care, and energy levels ",
      "Thoughts of suicide or fantasies about death ",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "If you’ve been living with depression for some time, you may feel guilty for your pain. The goal of Tinka Health isn’t to make you feel broken or sick. And we don’t believe there is a one-size-fits-all solution to helping you recover and enjoy life. The main goal of Tinka Health is for us to listen, better understand why you’re feeling this way, and identify strategies that can help you heal. Tinka Health will help you:  ",
    title3List: [
      "Feel safe opening up about your past, your struggles, and the difficult thoughts and emotions you’re experiencing.",
      "Teach you how to become more self-aware of when you’re feeling depressed and how you’re depression makes you feel or act.  ",
      "Help you learn to accept and love your authentic self and how to set boundaries and/or eliminate toxic influences in your life. ",
      "Set goals for your recovery and the future and identify achievable steps you can take to achieve them",
      "Help you gain control of your mind by developing stronger interpersonal, emotional regulation, and communication skills. ",
    ],
  },
  {
    id: "Grief-Trauma-Therapy",
    name: "Grief Trauma Therapy",
    id_sub: "Healing from loss and painful experiences",
    title1: "we are here to help you tackle that Grief & Trauma",
    title1Des:
      "Grief and trauma can be incredibly overwhelming. The death of a loved one or a traumatic experience can take you by surprise, and you may not know how to move forward. Tinka Health is here when you’re at you’re lowest – we’re here to listen without judgment and help you process your loss. ",
    btnText: "Let's Do It",
    title2: "Feel like you’ll never be “okay” again? ",
    title2Des:
      'How can you know if you\'re handling grief and trauma the "right" way? The truth is, there\'s no one "normal" way to feel—your grieving process is unique, and Tinka Health can help you heal in a way tailored to your needs. Most people will experience significant grief and trauma at some point, but not everyone processes these feelings the same way. You might feel stuck, hopeless, depressed, or afraid; the range of emotions can be overwhelming. Grief and trauma can have a profound and lasting impact on your life:',
    title2List: [
      "Feeling overwhelmed by intense emotions",
      "Rumination about the loss or traumatic event",
      "Feeling lethargic, unmotivated, or emotionally numb",
      "Changes in your sleeping or eating habits",
      "Increased anxiety, fear, or feelings of lost control",
      "New relationship issues, withdrawal, or irritability",
      "Difficulty focusing and/or dealing with stress",
      "Hopelessness, depression, or suicidal thoughts",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "The goal of grief and trauma therapy isn’t to dishonor the loved one you lost or invalidate your traumatic experience. If you’re struggling to move on or feel like life isn’t worth living anymore, we’re not here to judge or chastise you for your pain. We want to listen, help you process what happened, and help you heal. Tinka Health will help you:",
    title3List: [
      "Develop healthy coping mechanisms you can turn to when faced with triggers, depressing thoughts, or overwhelming emotions.",
      "Help your brain to reprocess traumatic or painful memories and minimize triggers through exposure and desensitization.",
      "Examine how grief and/or trauma are causing impairment in your life and help you gain stability and independence.",
      "Address complicated or painful reactions to grief and trauma, such as blame, guilt, denial, confusion, emptiness, or anger.",
    ],
  },
  {
    id: "Personality-Disorders",
    name: "Personality Disorders",
    id_sub: "Understand and reshape your patterns",
    title1: "we are here to help you tackle that personality disorder",
    title1Des:
      "Your personality is a unique and defining part of who you are. It encompasses your character traits, perspective, attitude, temperament, thought patterns, and values. A personality disorder is when a person’s unique traits cause significant issues and impairment. By working with Tinka Health to understands your disorder and how it’s disrupting your life, you can learn to better manage your symptoms and develop skills to better relate to others. ",
    btnText: "Let's Do It",
    title2: "Why do people develop personality disorders? ",
    title2Des:
      "Personality disorders can significantly affect your life by altering your perception of the world, hindering job stability, contributing to delinquent behavior, and damaging relationships. You might face emotional struggles, engage in self-harm or suicidal behaviors, act violently or lack empathy, experience low self-confidence, or exhibit perfectionism. Various factors may contribute to developing a personality disorder, such as genetics, brain changes, cultural influences, abuse history, and experiences in childhood and young adulthood. Signs often appear in early adulthood, but diagnosis may come later in life. Types of personality disorders include:",
    title2List: [
      "Narcissistic Personality Disorder: High sense of importance and entitlement; sensitive to criticism; deep need for success and admiration",
      "Histrionic Personality Disorder: Attention-seeking, flirtatious, and dramatic behaviors; easily influenced by others; obsessed with physical appearance",
      "Borderline Personality Disorder: Strong mood swings and intense emotions; impulsivity and paranoia; unstable relationships",
      "Antisocial Personality Disorder: Impulsive and high risk-taking behaviors; aggressive and violent tendencies; difficulties with empathy",
      "Schizotypal Personality Disorder: Eccentric or unusual thinking; paranoid or suspicious tendencies; overly superstitious; aloof or emotionally awkward; disconnected from reality",
      "Schizoid Personality Disorder: Limited range of emotional expression; prefers to be alone; little desire for sexual relationships; seems to lack motivations, pleasure, and goals",
      "Paranoid Personality Disorder: Difficulty trusting and confiding in others; difficulty relaxing; often see everyday situations as signs of danger",
      "Dependent Personality Disorder: Requiring others to make decisions or take responsibility for areas of your life; difficulty saying no and being alone; low self-confidence",
      "Avoidant Personality Disorder: Avoiding social activities; fear of ridicule or rejection; feeling lonely and inferior; highly sensitive to criticism",
      "Obsessive-Compulsive Personality Disorder: Need for control; desiring everything to be perfect; fearing mistakes; requiring order",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "Many personality disorders share symptoms with other mental health disorders, and it’s possible for you to simultaneously have a personality disorder and a mood disorder (e.g., you may have a histrionic personality disorder and also have depression). Tinka Health can help you determine if a personality disorder is primarily to blame for dysfunction in your life and we will provide long-term treatment to help you live a more functional, enjoyable life. Tinka Health will help by:",
    title3List: [
      "Helping you to look inward and identify what patterns in your thoughts, emotions, and behaviors are causing impairment in your life.",
      "Teaching you vital social skills to help you better understand yourself and others and interact with others in a healthy manner.",
      "Managing symptoms that are causing immediate distress or harm to yourself or others, such as depression or violent behavior.",
      "Learning to manage difficult emotions and cope in healthy ways, which may mean eliminating drugs or alcohol.",
    ],
  },
  {
    id: "Stress-Burnout",
    name: "Stress Burnout",
    id_sub: "Find balance and restore your energy",
    title1: "We're here to help you conquer stress and burnout like a pro!",
    title1Des:
      "Tired of dreading work each morning? Overwhelmed by your never-ending to-do list? Ready to reclaim your peace? Let Tinka Health help you manage stress and break free in both your work and personal life!",
    btnText: "Let's Do It",
    title2: "Do you feel like your life is spiraling out of control?",
    title2Des:
      "Burnout and constant stress often build up over time. You might be balancing a heavy workload, a full-time job, and demanding relationships, pushing yourself daily to perform at 110%—but now you've hit a breaking point. Despite efforts to “get back on track,” stress persists, impacting your health and well-being with symptoms like exhaustion, irritability, and feeling overwhelmed. you may also be experience the following:  ",
    title2List: [
      "Overwhelming feelings of panic and dread",
      "Difficulty falling asleep or staying asleep",
      "Unhealthy stress-reducing behaviors",
      "Social withdrawal or relationship issues",
      "Chest pain, muscle aches, and headaches",
      "Low motivation and poor work performance",
      "Mood changes, irritability, and emotional outbursts",
      "Poor nutrition, exercise, and other self-care habits",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "Burnout and stress won’t just resolve on their own. There are many factors – both internal and external – that impact your stress levels, how you react to stress, and why you’re feeling burnt out. Your overall mental health, your work environment, your coping mechanism, and your communication skills all have a role to play. Tinka Health will help you: ",
    title3List: [
      "Help you become aware of when you are beginning to feel overly stressed and showing signs of impending burnout.",
      "Identify what interpersonal and environmental factors are causing you to feel chronically stressed, burnt out, and overwhelmed.",
      "Develop healthy coping strategies and learn how to avoid negative thoughts and behavior patterns that contribute to poor mental health and more long-term stress.",
      "Learn how to set healthy boundaries in your life, recognize when your plate is full, and become comfortable with saying no.",
      "Evaluate your passions, values, and long-term goals and strategize how you can make purposeful changes to live a life you enjoy.",
    ],
  },
  {
    id: "Eating-Disorders",
    name: "Eating Disorders",
    id_sub: "Nurture a healthy relationship with food and yourself",
    title1: "We're here to help you conquer that eating disorders",
    title1Des:
      "Are you struggling with constant thoughts about food, your body, or self-image? Do you feel trapped in cycles of restrictive eating, bingeing, or purging? You’re not alone. Tinka Health is here to help you build a healthy, balanced relationship with food and yourself.",
    btnText: "Let's Do It",
    title2: "Do you feel like food and body image are controlling your life?",
    title2Des:
      "Eating disorders often start subtly but can take a heavy toll over time. You may find yourself caught in obsessive thoughts about food, feel guilty after meals, or see your self-worth tied to weight. Whether dealing with anorexia, bulimia, or binge eating disorder, the impact is real and far-reaching. Here are common effects of eating disorders ",
    title2List: [
      "Anxiety, guilt, or shame around eating",
      "Feeling detached from hunger cues or fullness",
      "Engaging in secretive eating habits",
      "Obsessive calorie counting, restrictive diets, or over-exercising",
      "Withdrawal from social settings due to food fears",
      "Physical symptoms such as fatigue, digestive problems, or nutrient deficiencies",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "Recovery is possible, and support makes all the difference. Tinka Health is here to help you understand the origins of your patterns with food, nurture a healthy self-image, and guide you towards genuine healing. Tinka Health will help you:",
    title3List: [
      "Identify the thoughts and triggers that lead to unhealthy eating patterns",
      "Cultivate a more positive self-image that isn’t tied to body size or weight",
      "Develop coping strategies that support healthy eating behaviors",
      "Work through underlying emotions or experiences connected to the disorder",
      "Establish a sustainable, balanced approach to eating and self-care",
    ],
  },
  {
    id: "Dissociative-Disorders",
    name: "Dissociative Disorders",
    id_sub: "Reconnect with yourself and find clarity",
    title1: "we are here to help you tackle that dissociative disorders ",
    title1Des:
      "Dissociative disorders cause you to feel disconnected from yourself, your loved ones, and the world around you as a means to escape difficult emotions, painful memories, or the effects of trauma. A dissociative disorder often develops as a coping mechanism when a child or young adult and causes significant problems in your life. Let's Tinka Health help you recover and cope. ",
    btnText: "Let's Do It",
    title2: "Do you often feel detached, disconnected, or out of place?",
    title2Des:
      "When under great stress or trauma, you may dissociate, feeling disconnected from your thoughts, body, and environment. While it's common to experience dissociation in stressful situations, those with dissociative disorders face this detachment more frequently, impacting daily life. You might feel as if your surroundings are unreal or even forget your identity, making it difficult to cope with stress and trauma healthily. There are several types of dissociative disorders you can be diagnosed with:",
    title2List: [
      "Dissociative identity disorder: Previously referred to as multiple personality disorder, DID can make it feel like you have more than personality inside your mind, which each has its own set of mannerisms, thought patterns, memories, and traits. You may switch personalities when faced with a stressful situation.",
      "Dissociative amnesia:  This type of dissociative disorder can cause you to forget certain events in your life and is not caused by physical trauma or issues with the brain. Often, the forgotten people, places, and events are connected to a past trauma. While you may not remember the event, you may still get triggered by things connected to the traumatic event. The amnesia can be so severe you may forget everything about your life, including your name and past.  ",
      "Depersonalization-derealization disorder: DDD can make you feel as if the world around you is not real. You might feel like you’re watching your life from the outside, that you or others aren’t really human, that time has slowed down or sped up, or that your body appears distorted. You may also feel disconnected from your emotions and feel numb. ",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "Living with a dissociative disorder can be very frightening – for yourself and your loved ones. They can cause serious impairment in your relationships and make it difficult to hold a job, manage your responsibilities, or care for yourself and others. Here is how Tinka Health can help you manage and recover from a dissociative disorder: ",
    title3List: [
      "Help you better understand your symptoms, the cause of your condition, and how it is impacting your life",
      "Talk with you about how you manage stress and difficult emotions and help you develop healthy ways of handling stressful situations",
      "Help you process, accept, and cope with past traumas in your childhood and adulthood that may be causing you to disassociate.",
      "Practice grounding techniques, such as deep breathing, sensory exercises, meditation, taking a walk, and moving or exercising",
      "Develop strategies to help you remember key events during an episode, such as leaving notes or keeping a journal",
    ],
  },
  {
    id: "Postpartum-Psychiatric-Disorders",
    name: "Postpartum Psychiatric Disorders",
    id_sub: "Embrace motherhood with mental well-being",
    title1:
      "we are here to help you tackle that postpartum psychiatric disorder",
    title1Des:
      "Pregnancy, childbirth, and postpartum recovery can feel like a roller coaster of emotions. If you’ve recently given birth and are experiencing significant difficulties with your mental health, you might have a postpartum psychiatric disorder. We’re here to help and support you during this wild time. Let's Tinka Health help you with postpartum depression and more.   ",
    btnText: "Let's Do It",
    title2: "Are you struggling with your mental health after childbirth? ",
    title2Des:
      "The postpartum period can be challenging for new mothers, often leading to mood changes due to poor sleep, physical pain, and hormonal shifts. While many experience temporary sadness or irritability known as the “baby blues,” some may develop more serious postpartum psychiatric disorders. These can severely impact the well-being of both mother and baby, resulting in difficulties with self-care, suicidal thoughts, intense anxiety, or psychosis. Various mental health disorders can affect women after childbirth. There are several mental health disorders that can affect women who’ve recently delivered a baby:",
    title2List: [
      "Postpartum depression: PPD causes symptoms of major depressive disorder, difficulties bonding with your baby, social withdrawal, feelings of worthlessness and hopelessness, thoughts of self-harm or suicide",
      "Postpartum anxiety: You may experience heart palpitations and physical anxiety symptoms, extreme fear about your baby’s safety, difficulties leaving home or trusting others with your child",
      "Birth trauma: An emotionally traumatic childbirth can cause symptoms of PTSD, such as hypervigilance, flashbacks, nightmares, avoidance behaviors, and panic.",
      "Symptoms of PTSD from a traumatic birth experience, flashbacks",
      "Postpartum psychosis: A severe postpartum condition that can cause delusions, mania, hallucinations, and mood instability",
    ],
    title3: "Tinka Health’s goals to help you:",
    title3Des:
      "No new mother wants to struggle with her mental health, and it can be incredibly difficult to adjust to being a new mother while struggling with a psychiatric disorder. Fortunately, therapy and psychiatric intervention can be very effective in helping you process what you’re feeling, recover, and enjoy these new precious moments with your new baby. Here’s how your Tinka Health can help: ",
    title3List: [
      "Evaluate your symptoms and determine if you’re showing signs and symptoms of a postpartum psychiatric disorder",

      "Discuss if medication could be beneficial for addressing symptoms of depression, anxiety, psychosis, and more",
      "Help you build a support group and get help from others to support your healing during this time",
      "Explore how other factors in your life, such as a difficult relationship or pre-existing conditions, might be affecting your mental health",
    ],
  },
];

export default servicesDataList;
